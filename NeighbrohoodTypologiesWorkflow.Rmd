...
mse_calc <- function(model = NULL, residuals = NULL, fitted_values = NULL, y = NULL){
  ...
  return(sum(residuals^2)/length(as_vector(residuals))) # return the mse
}
...
>>>>>>> b6968df04ce80896b09127126db8a3395148a0ec
```
...
model_brth_LQ <- betareg(teenbrth_pooled_female_mean ~ LQ_white_80 + LQ_black_80 + LQ_asian_80 + LQ_native_80 + LQ_hispanic_80, 
                         data = teenbirth)
...
prop_cv_inc <- cv_model(formula_call =cbind(jail_count, nonjail_count) ~ prop.nhwht80 + prop.nhblk80 + prop.asian80 + prop.hisp80
         , model = 'vglm', data = incar, k = 5, family = betabinomial)

ntype_cv_inc <- cv_model(cbind(jail_count, nonjail_count) ~ collapsed_type, 
                        model = 'vglm', data = incar, k = 5, family = betabinomial)

cluster_cv_inc <- cv_model(cbind(jail_count, nonjail_count) ~ wht.clst.dmmy + blk.clst.dmmy + as.clst.dmmy + hisp.clst.dmmy, 
                          model = 'vglm', data = incar, k = 5, family = betabinomial)

lq_cv_inc <- cv_model(cbind(jail_count, nonjail_count) ~ LQ_white_80 + LQ_black_80 + LQ_asian_80 + LQ_native_80 + LQ_hispanic_80, 
                     model = 'vglm', data = incar, k = 5, family = betabinomial)

cluster_prop_cv_inc <- cv_model(cbind(jail_count, nonjail_count) ~ wht.clst.dmmy + blk.clst.dmmy + as.clst.dmmy + hisp.clst.dmmy + prop.nhwht80 + prop.nhblk80 + prop.asian80 + prop.hisp80, 
                               model = 'vglm', data = incar, k = 5, family = betabinomial) 

null_cv_inc <- (incar %>% mutate(prop_jail = jail_count/nonjail_count) %>% summarise(mse = sum((mean(prop_jail)-prop_jail)^2)/nrow(incar)))[[1]]

tibble(
  proportion = prop_cv_inc,
  `neighborhood type` = ntype_cv_inc,
  cluster = cluster_cv_inc,
  `cluster + proportion` = cluster_prop_cv_inc,
  `location quotient` = lq_cv_inc
) %>% gather() %>%
ggplot(aes(value, key, color = key))+ # and plot
  geom_point()+
  geom_vline(aes(xintercept = null_cv_inc), color = 'red')+ # make a red vertical line for the mse of just guessing the mean value
  theme_minimal()+
  theme(legend.position = 'none')+
  ggtitle("Mean Squared Errors for different Models predicting Probability of Incarceration",
          "Red line indicates MSE for null model (always guessing the mean)")
