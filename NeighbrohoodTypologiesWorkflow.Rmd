...
cv_model <- function(formula_call, data, k = 5, model = 'glm', scoring_function = 'mse_calc', ...){
  ...
  for(i in 1:k){
    ...
    fold_model <- model(formula_call, data = df_fold, family = 'betabinomial')
    score[i] <- scoring_function(fitted_values = predict(fold_model, 
                                                         df[folds==i,] %>% select(all.vars(update(formula_call, 1 ~ .))), 
                                                         type = 'response'), 
                                 y = df[folds==i,] %>% select(all.vars(update(formula_call, . ~ 1))) %>% transmute(y = jail_count/nonjail_count))
  }
  mean(score)
}
```
...
null_cv_inc <- cv_model(cbind(jail_count, nonjail_count) ~ 1, 
                               model = 'vglm', data = incar, k = 5, family = betabinomial)
```
