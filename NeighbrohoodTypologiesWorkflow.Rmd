...
mse_calc <- function(model = NULL, residuals = NULL, fitted_values = NULL, y = NULL){
  if(is.null(residuals)){
    if(length(model$residuals)!=0){
      ...
      residuals <- model$y - model$fitted.values
    } else {
      stop("you need to supply a model with residuals or y and fitted values")
    }
  }
...
tibble(
  mean = mse_calc(fitted_values = mean(top_20$kfr_top20_pooled_pooled_mean), y = top_20$kfr_top20_pooled_pooled_mean),
  cluster = mse_calc(model_cluster),
