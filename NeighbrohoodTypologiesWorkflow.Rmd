...
cv_birth <- tibble( # make a tibble
  cluster = cv_model(teenbrth_pooled_female_mean ~ wht.clst.dmmy + blk.clst.dmmy + as.clst.dmmy + hisp.clst.dmmy + prop.nhwht80 + prop.nhblk80 + prop.asian80 + prop.hisp80, data = teenbirth, model = 'betareg'), # then for each model
...
cf_predict_20 %>% mutate(shape = if_else(model == 'cluster + proportion model', TRUE, FALSE)) %>% filter(!collapsed_type %in% c('0', 'none', 'empty')) %>%
ggplot(aes(x = pe, y = reorder(collapsed_type, -pe), alpha = .4))+
