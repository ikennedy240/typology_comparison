...
mse_calc <- function(model = NULL, residuals = NULL, fitted_values = NULL, y = NULL){
  if(is.null(residuals)){ # if we aren't given residuals
    if(length(model$residuals)!=0){
      ...
      residuals <- fitted_values - y  # then try and get them from function values
    } else if(is.null(fitted_values)){
...
sim_pe_se <- function(model, cfs, sims = 10000){
  ...
  for(i in 1:sims){ # for each sim
    ...
    probs[i,] <- predict(object, cfs) # and use the built in predict
  }
...
plot_df <- bind_rows(prep_avp_df(as.vector(model_inc_cluster@y), as.vector(model_inc_cluster@fitted.values), bins, "cluster"),
          ...
          prep_avp_df(as.vector(model_inc_cluster_prop@y), as.vector(model_inc_cluster_prop@fitted.values), bins, "racial proportion + clusters")) %>% 
  mutate(race_plus_clusters = if_else(model == "racial proportion + clusters", TRUE, FALSE)) # this mutate line adds a dummy for the model of interest
...
=======
    theme_minimal()+
    geom_abline(aes(slope = 1, intercept = 0), color = 'red')
>>>>>>> 66f8b86e6b08b781786acc62b2d3f068170fc321

# mse calc
mean_guess <- mse_calc(residuals = null_model@residuals) # assign the mean guess to a variable
tibble( # make a tibble
  cluster = mse_calc(residuals = model_inc_cluster@residuals[,1]), # then for each model
  loc_quotient = mse_calc(residuals = model_inc_lq@residuals[,1]),
  neighborhood_type = mse_calc(residuals = model_inc_ntype@residuals[,1]),
  proportion = mse_calc(residuals = model_inc_prop@residuals[,1]),
  cluster_prop = mse_calc(residuals = model_inc_cluster_prop@residuals[,1])
...
# create MLE plot using predict with type = response
tibble( # make a tibble
  cluster = mse_calc(fitted_values = predict(model_inc_cluster, data = incar, type = "response"), y = model_inc_cluster@y), # then for each model
  loc_quotient = mse_calc(fitted_values = predict(model_inc_lq, data = incar, type = "response"), y = model_inc_lq@y),
  neighborhood_type = mse_calc(fitted_values = predict(model_inc_ntype, data = incar, type = "response"), y = model_inc_ntype@y),
  proportion = mse_calc(fitted_values = predict(model_inc_prop, data = incar, type = "response"), y = model_inc_prop@y),
  cluster_prop = mse_calc(fitted_values = predict(model_inc_cluster_prop, data = incar, type = "response"), y = model_inc_cluster_prop@y)
) %>% gather() %>% # gather to long format
  ggplot(aes(value, key, color = key))+ # and plot
    geom_point()
    geom_vline(aes(xintercept = mean_guess), color = 'red')+ # make a red vertical line for the mse of just guessing the mean value
...
sim_pe_se(model_cluster_prop, cfs_20)
cf_predict <- bind_rows(
...
cf_predict %>% mutate(shape = if_else(model == 'cluster + proportion model', TRUE, FALSE)) %>% filter(!collapsed_type %in% c('0', 'none', 'empty')) %>%
ggplot(aes(x = pe, y = reorder(collapsed_type, -pe), alpha = .4))+
